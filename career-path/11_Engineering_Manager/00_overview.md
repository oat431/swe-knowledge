---
title: "Engineering Manager"
note_type: career-path-overview
career_family: people-leadership
level: manager
entry_from:
  - "[[career-path/02_Senior_Software_Engineer/00_overview|Senior Software Engineer]]"
  - "[[career-path/05_Tech_Lead/00_overview|Tech Lead]]"
next_paths:
  - "[[career-path/04_Principal_and_Distinguished_Engineer/00_overview|Principal and Distinguished Engineer]]"
  - "[[career-path/12_Technical_Program_Manager/00_overview|Technical Program Manager]]"
source_frameworks:
  - "[[SWEBOK v4 - Overview]]"
  - "[[PMBOK v8 - Overview]]"
  - "[[SEBoK v2 - Overview]]"
  - "[[BABOK v3 - Overview]]"
tags:
  - career-path
  - engineering-manager
  - people-leadership
  - management
---

# Engineering Manager

> **Positioning:** A people and delivery leadership path focused on creating the conditions for engineers and teams to deliver useful, sustainable results.

## What This Path Is

An Engineering Manager is responsible for the team's ability to deliver consistently, develop its people, maintain healthy working relationships, and operate within the organization's wider context.

The role is not simply a senior engineer with more meetings. Its center of gravity is people leadership: career development, hiring, coaching, performance, team health, communication, and organizational alignment. Technical understanding remains important, but the manager creates leverage through people and systems.

## Primary Outcomes

- A healthy, capable, and appropriately staffed engineering team
- Clear goals, priorities, and delivery expectations
- Engineers who receive useful feedback and career support
- Sustainable execution and transparent risk communication
- Collaboration with product, design, operations, security, and management
- Continuous improvement in team process and organizational effectiveness

## Capability Areas

| Capability | Manager behavior | Detailed notes |
|---|---|---|
| People development | Coaches, gives feedback, supports growth, and manages performance fairly | [[01_People_Development/00_overview]] |
| Team formation and health | Builds trust, clarity, psychological safety, and effective collaboration | [[02_Team_Formation_and_Health/00_overview]] |
| Hiring and staffing | Builds capability through hiring, onboarding, and workforce planning | [[03_Hiring_and_Staffing/00_overview]] |
| Delivery leadership | Creates clarity around scope, priority, risk, and progress | [[04_Delivery_Leadership_for_Managers/00_overview]] |
| Technical context | Participates meaningfully in technical decisions without becoming the sole decision maker | [[06_Technical_Context_for_Managers/00_overview]] |
| Organizational awareness | Connects team work to strategy, value, and change | [[05_Organizational_Awareness_and_Influence/00_overview]] |
| Communication | Cascades context and represents the team accurately | [[07_Manager_Communication/00_overview]] |

## Typical Progression

```mermaid
flowchart LR
    SENIOR["Senior Software Engineer"] --> LEAD["Lead people and delivery"]
    LEAD --> TEAM["Manage one team"]
    TEAM --> GROUP["Manage multiple teams or managers"]
    GROUP --> DIRECTOR["Engineering Director and beyond"]
```

## Signals for Moving Forward

- You gain satisfaction from helping other people succeed.
- You can make team decisions that are fair, clear, and aligned with organizational needs.
- You can handle difficult conversations without avoiding them or making them personal.
- You are willing to measure your success through team outcomes rather than personal code output.
- You can preserve technical credibility while delegating implementation ownership.

## Evidence to Build

- Team charter and working agreement
- Hiring or onboarding plan
- Career development plan
- One-to-one structure
- Team health or capability assessment
- Delivery and risk review
- Performance feedback examples
- Process improvement proposal
- Stakeholder communication plan

## Nearby Paths

- [[career-path/05_Tech_Lead/00_overview|Tech Lead]]: technical system ownership
- [[career-path/02_Senior_Software_Engineer/00_overview|Senior Software Engineer]]: individual-contributor foundation
- [[career-path/03_Staff_Engineer/00_overview|Staff Engineer]]: cross-team technical influence
- [[career-path/12_Technical_Program_Manager/00_overview|Technical Program Manager]]: cross-team initiative delivery
- [[career-path/13_Project_and_Program_Manager/00_overview|Project or Program Manager]]: formal project and program management

## Suggested Future Note Route

1. [[software-engineering-note/09_Software_Engineering_Management/Software Engineering Management Overview]]
2. [[software-engineering-note/09_Software_Engineering_Management/01_Managing_the_Human_Resource]]
3. [[software-engineering-note/09_Software_Engineering_Management/03_The_Right_People]]
4. [[software-engineering-note/09_Software_Engineering_Management/04_Growing_Productive_Teams]]
5. [[software-engineering-note/14_Software_Engineering_Professional_Practice/02_Group_Dynamics_and_Psychology]]
6. [[software-engineering-note/14_Software_Engineering_Professional_Practice/03_Communication_Skills]]
7. [[PMBOK v8 - Overview]]
8. [[SEBoK v2 - Overview]]

## Sources

- [Engineering Ladders: Engineering Manager](https://www.engineeringladders.com/EngineeringManager.html)
- [Engineering Ladders: Tech Lead versus Engineering Manager](https://www.engineeringladders.com/TechLead-EngineeringManager.html)
- [U.S. Bureau of Labor Statistics: Computer and Information Systems Managers](https://www.bls.gov/ooh/management/computer-and-information-systems-managers.htm)
- [[SWEBOK v4 - Overview]]
- [[PMBOK v8 - Overview]]

## Related

- [[00_Career_Path_Overview]]
- [[career-path/02_Senior_Software_Engineer/00_overview|Senior Software Engineer]]
- [[career-path/05_Tech_Lead/00_overview|Tech Lead]]
- [[career-path/12_Technical_Program_Manager/00_overview|Technical Program Manager]]
